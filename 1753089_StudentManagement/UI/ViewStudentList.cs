...
namespace UI
{
    public partial class ViewStudentList : Form
    {
        ...
        {
            this.liststudent.BeginUpdate();
            this.liststudent.Clear();

            this.liststudent.Columns.Add("MSSV", 100);
            this.liststudent.Columns.Add("CMND", 100);
            this.liststudent.Columns.Add("Họ tên", 250);
            this.liststudent.Columns.Add("Lớp", 80);
            this.liststudent.Columns.Add("Ngày sinh", 100);
            this.liststudent.Columns.Add("Địa chỉ", 250);
            this.liststudent.Columns.Add("Giới tính", 100);

            var students = new BLL.BLL().ViewStudentsListinClassFromDB(this.classcb.SelectedItem.ToString());

            foreach (Student student in students)
            {
                String[] stu = new String[7];
                stu[0] = student.studentid;
                stu[1] = student.socialid;
                stu[2] = student.studentname;
                stu[3] = student.classid;
                stu[4] = student.dob.ToString();
                stu[5] = student.address;
                stu[6] = student.sex;

                ListViewItem item = new ListViewItem(stu);
                this.liststudent.Items.Add(item);
            }
            this.liststudent.View = View.Details;
            this.liststudent.EndUpdate();
            this.classcb.Text = this.classcb.SelectedItem.ToString();
        }
        ...
        private void ViewStudentList_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Hide();
